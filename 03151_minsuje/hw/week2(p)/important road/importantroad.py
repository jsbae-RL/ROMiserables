...
class ImportRoad():
    ...
    def __shortest_path(self):
        ...
        '''
        #다익스트라 알고리즘을 이용하여 최단 시간을 알아야 해서 경로의 최단시간만 알면 됨됨
        
        #queue의 역할을 할 변수 location_info 작성-시간과 지역역 값을 확인하며 리스트에 튜플로 작성한 이유는
        
        # 최단 시간 변수
        #shorest_time 에 location키의 벨류 값은 short_time의 최대값을 넣어야 최초 비교때 최솟 값을 넣을 수 있음
        #따라서 도로의 최소수(1-2-3-4-5이런식으로 하나씩만 연결됬고 모든 도로를 지나야 될때 이게 제일 큰값임임)*2*트레픽 최대값을 넣어줌.
